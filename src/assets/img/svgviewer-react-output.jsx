import * as React from "react";

function SVGComponent1  (props) {
  <svg
    id="Layer_1"
    xmlns="http://www.w3.org/2000/svg"
    xmlnsXlink="http://www.w3.org/1999/xlink"
    x="0px"
    y="0px"
    viewBox="0 0 512 512"
    style={{
      enableBackground: "new 0 0 512 512",
    }}
    xmlSpace="preserve"
    {...props}
  >
    <polygon
      style={{
        fill: "#E6E6E6",
      }}
      points="14.273,112.52 14.273,234.106 14.273,383.665 381.737,383.665 438.024,383.665  497.727,383.665 497.727,234.106 497.727,112.52 "
    />
    <polygon
      style={{
        fill: "#F7B239",
      }}
      points="512,91.382 512,404.803 482.093,374.896 482.093,121.289 "
    />
    <g>
      <polygon
        style={{
          fill: "#E09B2D",
        }}
        points="512,91.382 482.093,121.289 29.907,121.289 0,91.382  "
      />
      <polygon
        style={{
          fill: "#E09B2D",
        }}
        points="482.093,374.896 373.604,374.896 346.422,374.896 29.907,374.896 0,404.803  346.422,404.803 373.604,404.803 512,404.803  "
      />
    </g>
    <path
      style={{
        fill: "#F95428",
      }}
      d="M404.504,404.803l-4.582-3.326l-4.57,3.326h-0.012l-21.736,15.815v-15.815v-67.047v-22.514 c0.969,0.263,1.89,0.598,2.727,1.029c5.156,2.632,13.741,15.737,14.654,10.144c9.264-56.773,11.424-4.235,17.071-5.132 c4.725-0.742,10.515,1.376,15.312,0.467h2.883v16.006v67.047v15.803L404.504,404.803z"
    />
    <polygon
      style={{
        fill: "#F7B239",
      }}
      points="29.907,121.289 29.907,374.896 0,404.803 0,91.382 "
    />
    <g>
      <rect
        x={166.878}
        y={162.584}
        style={{
          fill: "#666666",
        }}
        width={178.243}
        height={17.944}
      />
      <rect
        x={192}
        y={231.967}
        style={{
          fill: "#666666",
        }}
        width={128}
        height={17.944}
      />
      <rect
        x={135.776}
        y={196.079}
        style={{
          fill: "#666666",
        }}
        width={240.449}
        height={17.944}
      />
      <rect
        x={135.776}
        y={314.485}
        style={{
          fill: "#666666",
        }}
        width={88.523}
        height={17.944}
      />
    </g>
    <path
      style={{
        fill: "#F7B239",
      }}
      d="M373.604,352.383c-4.749-1.376-10.647-1.412-14.092-4.869c-4.163-4.163-3.361-11.843-5.898-16.819 c-2.632-5.156-9.271-9.056-10.156-14.654c-0.885-5.658,4.247-11.424,5.144-17.083c0.873-5.587-2.249-12.668,0.383-17.824 c2.536-4.976,10.037-6.627,14.2-10.778c4.163-4.163,5.814-11.676,10.79-14.212c5.156-2.62,12.226,0.502,17.824-0.383 c5.646-0.885,11.412-6.017,17.071-5.132c5.599,0.885,9.498,7.524,14.654,10.156c4.976,2.536,12.668,1.723,16.819,5.886 c4.163,4.163,3.362,11.855,5.898,16.831c2.632,5.156,9.271,9.056,10.156,14.654c0.885,5.646-4.247,11.424-5.144,17.071 c-0.873,5.599,2.249,12.668-0.383,17.824c-2.536,4.976-10.037,6.627-14.2,10.79c-4.163,4.163-5.814,11.664-10.778,14.2 c-0.802,0.407-1.651,0.682-2.524,0.849c-4.797,0.909-10.587-1.208-15.312-0.467c-5.646,0.897-11.412,6.029-17.071,5.132 c-5.598-0.873-9.498-7.513-14.654-10.144C375.495,352.981,374.573,352.646,373.604,352.383z"
    />
    <path
      style={{
        fill: "#F95428",
      }}
      d="M417.495,289.531c9.702,9.702,9.702,25.433,0,35.122c-9.702,9.702-25.433,9.702-35.134,0 c-9.702-9.69-9.702-25.421,0-35.122C392.063,279.829,407.794,279.829,417.495,289.531z"
    />
    <g />
    <g />
    <g />
    <g />
    <g />
    <g />
    <g />
    <g />
    <g />
    <g />
    <g />
    <g />
    <g />
    <g />
    <g />
  </svg>
    }

export default SVGComponent1;
